...
def train(args, category, rotate_90=False, random_rotate=0):
    ...
    de_st_optimizer = torch.optim.SGD(
        [
            {"params": model.student_dec.parameters(), "lr": args.lr_de_st},
        ],
        lr=0.4,
        momentum=0.9,
        weight_decay=1e-4,
    ...
    st_optimizer = torch.optim.SGD(
        [
            {"params": model.student_enc.parameters(), "lr": args.lr_de_st},
        ],
        lr=0.4,
        momentum=0.9,
        weight_decay=1e-4,
    ...
    while flag:
        for _, sample_batched in enumerate(dataloader):
            ...
            if global_step < args.de_st_steps:
                model.student_enc.train()
                model.student_dec.train()
                model.segmentation_net.eval()
            else:
                model.student_enc.eval()
                model.student_dec.eval()
                model.segmentation_net.train()

            output_segmentation, output_stenc, output_de_stdec, output_stenc_list, output_de_stdec_list = model(
                img_aug, img_origin
            )
            ...
            cosine_loss_val_enc = cosine_similarity_loss(output_stenc_list)
            cosine_loss_val_dec = cosine_similarity_loss(output_de_stdec_list)
            focal_loss_val = focal_loss(output_segmentation, mask, gamma=args.gamma)
            l1_loss_val = l1_loss(output_segmentation, mask)

            if global_step < args.de_st_steps:
                total_loss_val = cosine_loss_val_enc + cosine_loss_val_dec
            ...
            visualizer.add_scalar("cosine_loss_enc", cosine_loss_val_enc, global_step)
            visualizer.add_scalar("cosine_loss_dec", cosine_loss_val_dec, global_step)
            ...
            if global_step % args.eval_per_steps == 0:
                val_auc, val_ap = evaluate(args, category, model, visualizer, global_step)
                if val_auc > best_auc:
                    torch.save(
                        model.state_dict(), os.path.join(args.checkpoint_path, run_name + '_best_auc' + ".pckl")
                    )
                if val_ap > best_ap:
                    torch.save(
                        model.state_dict(), os.path.join(args.checkpoint_path, run_name + '_best_ap' + ".pckl")
                    )

            if global_step % args.log_per_steps == 0:
                if global_step < args.de_st_steps:
                    print(
                        f"Training at global step {global_step}, cosine loss enc: {round(float(cosine_loss_val_enc), 4)}, cosine loss dec: {round(float(cosine_loss_val_dec), 4)}"
                    )
                else:
                    print(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_name_head", type=str, default="SimSTSeg_MVTec")
